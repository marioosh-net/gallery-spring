<%@ include file="/WEB-INF/templates/taglibs.jsp" %>
<!--
<security:authorize ifAnyGranted="ROLE_USER,ROLE_ADMIN">
	<a href="/logout.html"><spring:message code="button.logout"/></a>
</security:authorize>
-->
<security:authorize ifNotGranted="ROLE_USER,ROLE_ADMIN">
	<form:form cssClass="sform" id="jf" name="f" action="j_spring_security_check" method="post">
		<c:if test="${!empty param.loginfail}">
			<div class="modal-message">
				<span class="errors"><spring:message code="text.loginError"/></span>
			</div>
		</c:if>	
		<input type="text" id="username" name="j_username" value="" size="5"/>
		<input type="password" name="j_password" value="" size="6" />
		<input type="checkbox" name="_spring_security_remember_me" style="width: 20px;"/>
		<a href="javascript:;" onclick="jQuery('#jf').submit();"><spring:message code="button.login"/></a>
		<input type="submit" class="hiddensubmit"/>
	</form:form>
</security:authorize>
