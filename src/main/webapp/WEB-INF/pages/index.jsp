...
<t:layout>
	...
	<div class="left leftfixed">		
		...
		<div class="frame-albums" style="padding-bottom: 0px;">
	...
	<div class="left rightfixed">
		...
		<security:authorize ifAnyGranted="ROLE_ADMIN">
			<div id="main-funcs">
				<a href="#" title="<spring:message code='text.description.scan'/>" onclick="openModal(this); return false;" id="scan-trigger" class="modalInput modalInputClick" rel="#yesnoadmin" rev="loadingIcon('#scan-trigger'); jQuery('#log').html(''); openOverlay('#logmodal', null); jQuery.get('<c:url value="/app/scan.html?refresh=1"/>', function(data){/*openOverlay('#scaned', data);*/ jQuery('#scan-trigger').text('<spring:message code="button.scan"/>');})"><spring:message code="button.scan"/></a>
				<a href="#" title="<spring:message code='text.description.scanOnlyNew'/>" onclick="openModal(this); return false;" id="scan-trigger" class="modalInput modalInputClick" rel="#yesnoadmin" rev="loadingIcon('#scan-trigger'); jQuery('#log').html(''); openOverlay('#logmodal', null); jQuery.get('<c:url value="/app/scanOnlyNew.html"/>', function(data){/*openOverlay('#scaned', data);*/ jQuery('#scan-trigger').text('<spring:message code="button.scan"/>');})"><spring:message code="button.scanOnlyNew"/></a>
		...
		<%-- footer --%>		
		<div id="footer-right">
			Inspired by <a href="http://minishowcase.net">minishowcase</a>. Powered by <a href="http://www.springsource.org/"><img style="vertical-align: text-bottom;" src="<c:url value="/images/spring2.png"/>"/></a>&nbsp;|&nbsp;<a href="http://ramtatam.com">ramtatam.com</a><%-- &nbsp;|&nbsp;<a href="<t:server-https/><c:url value='/'/>">https</a>--%>
