			...
			<div class="left ainfo">
				...
				<div>
					<security:authorize ifAnyGranted="ROLE_ADMIN">
						...
						<a href="#" onclick="openModal(this); return false;" class="modalInput modalInputHref" rel="#delalb" rev="<c:url value="/app/deletealbum.html?id=${album.id}"/>"><img class="icon middle" height="16" width="16" src="<c:url value="/images/delete.png"/>"/><spring:message code="button.delete"/></a><br/>
						<a href="#" title="<spring:message code='text.description.scan'/>" onclick="openModal(this); return false;" id="scan-trigger-alb" class="modalInput modalInputClick" rel="#yesnoalbum" rev="loadingIcon('#scan-trigger-alb'); openOverlay('#logmodal', null); jQuery.get('<c:url value="/app/scan.html?refresh=1"/>', { path: '${album.path}'}, function(data){/*openOverlay('#scaned', data);*/ jQuery('#scan-trigger-alb').text('<spring:message code="button.scan"/>');})"><spring:message code="button.scan"/></a>
						<a href="#" title="<spring:message code='text.description.reload'/>" onclick="openModal(this); return false;" id="reload-trigger" class="modalInput modalInputClick" rel="#yesnoalbum" rev="loadingIcon('#reload-trigger'); jQuery.get('<c:url value="/app/reload.html"/>', { id: '${album.id}'}, function(data){openOverlay('#scaned', data); jQuery('#reload-trigger').text('<spring:message code="button.reload"/>');})"><spring:message code="button.reload"/></a>
						<a href="#" title="<spring:message code='text.description.refresh'/>" onclick="openModal(this); return false;" id="refresh-trigger" class="modalInput modalInputClick" rel="#yesnoalbum" rev="loadingIcon('#refresh-trigger'); jQuery.get('<c:url value="/app/refresh.html"/>', { id: '${album.id}'}, function(data){if(data == '0') {openOverlay('#scaned', 'OK');} else {openOverlay('#scaned', 'Error');} jQuery('#refresh-trigger').text('<spring:message code="button.refresh"/>');})"><img class="middle" src="<c:url value="/images/refresh.png"/>" height="16" width="16"/><spring:message code="button.refresh"/></a>
						</div>
